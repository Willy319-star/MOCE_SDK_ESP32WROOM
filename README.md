...
5. 编译与烧录。Agent Web 界面会根据系统自动选择 Linux/macOS 的 `tools/*.sh` 或 Windows 的 `tools/*.ps1`。手动执行时：
    ...
    Linux/macOS：
    ```bash
    ./tools/build.sh project/<your_project_name> esp32s3 my_board_esp32s3
    ./tools/flash.sh project/<your_project_name> --target esp32s3 --board my_board_esp32s3 --port /dev/ttyUSB0
    ```

    Windows PowerShell：
    ```powershell
    ./tools/build.ps1 project/<your_project_name> esp32s3 my_board_esp32s3
    ./tools/flash.ps1 project/<your_project_name> --target esp32s3 --board my_board_esp32s3 --port COM3
    ```
## 常见问题
